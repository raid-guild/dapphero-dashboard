...
export async function readContract(
  ...
  const transactions = await arweave.arql(arql);
  ...
  const unconfirmed = await batcher(transactions);
  ...
  const txInfos = unconfirmed.filter(
    (x) => x && x.info.confirmed && x.info.confirmed.block_height <= height,
  ) as InteractionTx[];
