...
const getHtmlFromPieces = pieces => pieces.map(({
  html
}) => html);
...
const generateHtmlPieces = (abi = [], contractName) => {
  return filterAnonymousMethods(abi).map(method => {
    ...
    const newInputs = [...inputs, ...(isPayable ? [{
      name: 'EthValue',
      type: 'EthValue',
      payable: 'true'
    }] : [])];
    const hasMoreThanOneAnonymousInput = newInputs.map(({
      name
    }) => name).filter(name => name === '').length > 1;
...
const getHtmlPiecesFromTransactionMethods = (abi, contractName) => {
  var _ref3, _ref4, _abi2;
  _abi2 = abi;
  _ref4 = getMethods(_abi2);
  _ref3 = getTransactionMethods(_ref4);
